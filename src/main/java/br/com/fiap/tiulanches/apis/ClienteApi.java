...
import br.com.fiap.tiulanches.controllers.ClienteController;
...
public class ClienteApi {	
	...
	@Operation(summary = "Lista todos os clientes cadastrados", description = "O retorno é paginado o padrão são 10 registros por página", tags = {"Cliente"})
	@ApiResponses(value = {
	...
	public ResponseEntity<Page<ClienteDto>> consultar(@ParameterObject @PageableDefault(size=10) Pageable paginacao){
		...
		Page<ClienteDto> page = controller.consultar(paginacao);
		
		return ResponseEntity.ok(page);
	}
	
	@GetMapping(value = "/{cpf}", produces = MediaType.APPLICATION_JSON_VALUE)
	@Operation(summary = "Retorna dados do cliente", description = "Retorna todos os dados do cliente do CPF informado", tags = {"Cliente"})
	@ApiResponses(value = {
	...
	@ApiResponses(value = {
			...
			@ApiResponse(responseCode = "400", description = "Falha, produto vínculado a um pedido", content=@Content(schema = @Schema(example = ClienteResponseSwagger.BADREQUESTVINCULADOPEDIDO))),
			@ApiResponse(responseCode = "404", description = "Falha, cliente não encontrado", content=@Content(schema = @Schema(hidden = true)))
	})
